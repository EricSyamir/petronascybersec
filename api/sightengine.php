...
class SightengineAPI {
    ...
    public function processDetection($filePath, $results, $reportId = null) {
        ...
        try {
            ...
            $stmt = $pdo->prepare("SELECT id, detection_results FROM deepfake_detections WHERE file_hash = ?");
            $stmt->execute([$fileHash]);
            $existing = $stmt->fetch();
            
            if ($existing) {
                // Parse the stored detection_results and re-analyze to get analysis object
                $storedResults = json_decode($existing['detection_results'], true);
                if ($storedResults) {
                    $analysis = $this->analyzeResults($storedResults);
                    return [
                        'id' => $existing['id'],
                        'detection_results' => $storedResults, // Return as object, not string
                        'analysis' => $analysis
                    ];
                }
            }
            ...
            // Save to database
            $stmt = $pdo->prepare("INSERT INTO deepfake_detections 
                (file_path, file_hash, detection_results, confidence_score, is_deepfake, report_id) 
                VALUES (?, ?, ?, ?, ?, ?)");
            ...
            $detectionId = $pdo->lastInsertId();
            
            // Log the detection
            logAudit('deepfake_detection', 'deepfake_detections', $detectionId, null, [
                'file_hash' => $fileHash,
    ...
    public function getDetectionStats($timeframe = '24h') {
        ...
        $stmt = $pdo->prepare("
            SELECT 
                ...
                SUM(CASE WHEN is_deepfake = TRUE THEN 1 ELSE 0 END) as deepfakes_found,
