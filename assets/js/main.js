...
class PetronasMainApp {
    ...
    setupAuthenticatedFeatures() {
        ...
        // Skip statistics updates on osint-monitor page
        if (!window.location.pathname.includes('osint-monitor')) {
    ...
    setupNotifications() {
        ...
        // Disabled for development - uncomment and create sw.js for production
        /*
    ...
    startPeriodicUpdates() {
        // Update statistics every 5 minutes - DISABLED for osint-monitor
        // Only run if we're not on osint-monitor page
        if (!window.location.pathname.includes('osint-monitor')) {
            setInterval(() => {
