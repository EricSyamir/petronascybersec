...
class OSINTMonitor {
    ...
    updateStatsDisplay(stats) {
        document.getElementById('criticalCount').textContent = stats.critical_threats || 0;
        document.getElementById('highCount').textContent = stats.high_threats || 0;
        document.getElementById('totalCount').textContent = stats.total_threats || 0;
        
        // Update change indicators (simulate for demo)
        document.getElementById('criticalChange').textContent = 'Last 24h';
        document.getElementById('highChange').textContent = 'Last 24h';
        document.getElementById('totalChange').textContent = 'Last 24h';
    }
    ...
    async searchThreats() {
        ...
        try {
            ...
            if (result.success) {
                this.currentThreats = result.threats;
                this.displayThreats(result.threats, true);
            } else {
            ...
            this.showAlert('Failed to search threats', 'danger');
        } finally {
            this.isLoading = false;
        }
    }
    ...
    async loadTrendingKeywords() {
        try {
            ...
            if (result.success) {
                this.displayTrendingKeywords(result.keywords);
            }
